...
<head>
  ...
  $sync_time = date('Y-m-d', filemtime("GABCs/$max_id.gabc"));
  ?>
</head>
<body>
  ...
  <p>Last sync with <a target="_blank" href="https://gregobase.selapa.net/">GregoBase</a>: <?php echo $sync_time ?>. Max GregoBase ID: <?php echo $max_id; ?>.</p>
  ...
  <ol start="0">
    ...
    if (isset($id) and isset($n)) {
        if ($id < 0 or $id > $max_id)
                die('<font color="red">GregoBase ID must be >0 and ≤' . $max_id . '.</font>');
        else if ($n < 0 or $n > 64)
